...
export default function EditVideoForm({
  ...
  const syncMediaErrors = useMemo(
    () => validateMediaSync(newFiles, selectedPlatforms, "VIDEO"),
  ...
  useEffect(() => {
    ...
    validateMediaFiles(newFiles, selectedPlatforms, "VIDEO")
  ...
  async function submit() {
    ...
    if (!hasMedia) {
      toast.error("Please keep or upload at least one video");
      return;
    }
      ...
      <div className="space-y-2">
        ...
        {slotsForNew > 0 && (
          <MediaUploader
            ...
            accept="video/*"
            label={newFiles.length === 0 ? "Upload New Video" : "Add Another Video"}
