...
export function PublishedPostCard({ post }: { post: PostResponse }) {
  ...
  const accountsToShow =
    post.connectedAccounts?.slice(0, maxVisibleAccounts) || [];
  const remainingCount =
    (post.connectedAccounts?.length || 0) - maxVisibleAccounts;
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex-1 flex flex-col">
        <h3
          className="text-base font-semibold text-card-foreground mb-2 line-clamp-2 
      ...
      >
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span>
            {post.connectedAccounts?.length || 0} account
            {post.connectedAccounts?.length !== 1 ? "s" : ""}
          </span>
        </div>
