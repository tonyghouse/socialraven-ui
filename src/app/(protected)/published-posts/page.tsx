...
export default function PublishedPostsPage() {
  ...
  const loadPosts = useCallback(async (page: number, isManualRefresh = false) => {
    ...
    try {
      ...
      setError("Unable to load published posts. Please try again.");
    } finally {
      setLoading(false);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {loading && posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm text-muted-foreground">Loading Published posts...</p>
          </div>
        ) : posts.length > 0 ? (
          <>
            {/* Posts Grid */}
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center mb-4">
              <CalendarCheck2 className="w-8 h-8 text-muted-foreground" />
            </div>
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm mb-6">
              Your Published posts will appear here. Start planning your content!
            </p>
