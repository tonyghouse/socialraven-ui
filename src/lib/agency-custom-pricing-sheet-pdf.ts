...
function measureText(text: string, size: number, font: "F1" | "F2") {
  const widthFactor = font === "F2" ? 0.58 : 0.53;
  return text.length * size * widthFactor;
}

function wrapText(text: string, maxWidth: number, size: number, font: "F1" | "F2") {
...
export function generateAgencyCustomPricingSheetPdf() {
  const ops: string[] = [];
  ...
  function setFillColor([r, g, b]: readonly number[]) {
    push(`${r} ${g} ${b} rg`);
  }

  function setStrokeColor([r, g, b]: readonly number[]) {
    push(`${r} ${g} ${b} RG`);
  }
  ...
  function drawLine(
    ...
    x2: number,
    y2: number,
    { width = 1, color = COLORS.line }: { width?: number; color?: readonly number[] } = {}
  ) {
  ...
  function drawText(
    ...
    x: number,
    y: number,
    { font = "F1", size = 12, color = COLORS.ink }: { font?: "F1" | "F2"; size?: number; color?: readonly number[] } = {}
  ) {
    setFillColor(color);
  ...
  function drawBulletList(
    ...
    y: number,
    width: number,
    { size = 11, color = COLORS.ink, leading = 14 }: { size?: number; color?: readonly number[]; leading?: number } = {}
  ) {
    let cursor = y;
    items.forEach((item) => {
